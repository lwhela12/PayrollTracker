...
export class DatabaseStorage implements IStorage {
  ...
  async getAuditLogsByEmployer(employerId: number, limit: number = 100): Promise<any[]> {
    return await db
      .select({
        id: auditLogs.id,
        userId: auditLogs.userId,
        employerId: auditLogs.employerId,
  ...
  async getDashboardStats(employerId: number, payPeriodId: number): Promise<any[]> {
    ...
    const query = `
      ...
      SELECT
        ...
        COALESCE(rt.mileage,0) + COALESCE(tc.mileage,0) + COALESCE(dm.daily_mileage,0) as "mileage",
      ...
      LEFT JOIN daily_mileage_totals dm ON e.id=dm.employee_id
